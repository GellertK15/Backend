...
namespace WindowsFormsApp1
{
    ...
    public partial class Form_Main : Form
    {
        ...
        private async void button_Create_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                long daikId = await inserDiak(ujDiak);
                if (daikId >= 0) {
                    textBox_id.Text = daikId.ToString();
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Hiba az új diák létrehozásakor: " + ex.Message);
            }
        }

        private async Task<long> inserDiak(Diak ujDiak)
        {
            long ujId = -1;
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    throw new Exception("API hiba az új diák létrehozásakor: " + response.StatusCode);
                }
                var responseString = await response.Content.ReadAsStringAsync();
                var createdDiak = JsonConvert.DeserializeObject<Diak>(responseString);
                ujId = createdDiak.Id;
            ...
            {
                throw new Exception("Hiba az új diák létrehozásakor: " + ex.Message);
            }
        }
        ...
        private async void button_update_ClickAsync(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Hiba a diák frissítése során: " + ex.Message);
            }
        }
        ...
        private async Task<bool> updateDiak(Diak frissitettDiak)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                throw new Exception("API hiba a diák frissítése során: " + response.StatusCode);
            }
            else
            {
